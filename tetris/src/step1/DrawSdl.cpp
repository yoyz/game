...
void DrawSdl::draw_block(int x,int y)
{
  SDL_Rect sqrRect;
  sqrRect.x=x;
  sqrRect.y=y;
  ...
  SDL_FillRect(screen , &sqrRect , 0x661122);
}
...
void DrawSdl::draw_border()
{
  ...
  //  board_position.y=92;
  board_position.x=(SCREEN_SIZE_X*1)/3;
  board_position.y=(SCREEN_SIZE_Y*1)/3;
  for (int i=0;i<BLOCK_LENGTH;i++)
    {
      //SDL_BlitSurface(grey_box, NULL, screen, &board_position);
      draw_block(board_position.x,board_position.y);
  ...
  board_position.x=(SCREEN_SIZE_X*2)/3;
  board_position.y=(SCREEN_SIZE_Y*1)/3;
  for (int i=0;i<BLOCK_LENGTH;i++)
    {
      //SDL_BlitSurface(grey_box, NULL, screen, &board_position);
  ...
  board_position.x=(SCREEN_SIZE_X*1)/3;
  board_position.y=(SCREEN_SIZE_Y*4)/5;
  ...
  for(int i = 0;i< 4; i++)
    {
      ...
      draw_block(position.x,position.y);
    }
}
  ...
  for(int i = 0;i< 4; i++)
    {
      ...
      pos.x = SQUARE_SIZE*(block_v[i].get_x()-2);
      pos.y = SQUARE_SIZE*(block_v[i].get_y()+3);
      
      draw_block(pos.x,pos.y);
    }
}
