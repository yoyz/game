...
{
  int current_shape_number=(rand() % 5)+1;
  int next_shape_number=(rand() % 5)+1;
  current_shape = new  Shape(current_shape_number,current_shape_number); 
  next_shape    = new  Shape(next_shape_number,next_shape_number); 
}
...
{    
  ...
  while (cont)
      ...
      if (event_to_draw)
	{
	  ...
	  on_screen.draw_dead_shape_on_board(GB);
	  ...
	  on_screen.draw_active_shape_on_board(current_shape);
